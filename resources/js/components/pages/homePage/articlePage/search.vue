        ...
        <div v-if="isLoaded" class="w-full flex justify-center space-x-7">
            <div class="lg:w-5/12 md:w-10/12 w-full lg:px-0 px-2 pt-10">
                <div class="w-full border-b-2 border-sky-300">
                    <p class="text-4xl text-left blur-none ">Search Result for "<span class="text-sky-800">{{ this.$route.params.search }}</span>"</p>
                </div>
                <div class="w-full pt-10">
                    <div class="w-full">
                        <div v-for="(searched, key) in this.searchArticleList" class="flex block border-b-2 border-sky-900/60 pb-3 mb-5">
                            <img @click="gotoArticle(searched)" class="cursor-pointer bg-sky-200 object-cover object-center blur-none w-24 h-24 " :src="`/uploads/low/${searched.image}`">
                            <div class="block w-fit px-1 overflow-hidden">
                                <p @click="gotoArticle(searched)" class="cursor-pointer text-sm blur-none antialiased font-semibold text-sky-600  truncate">{{ searched.title }}</p>
                                <p class="text-xs blur-none"> {{ searched.type.name }} | {{ searched.date }}</p>
                                <p class="blur-none antialiased text-xs text-justify line-clamp-3 leading-relaxed">By {{ searched.author }}, {{ searched.article }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="w-2/12 pt-10 hidden lg:block">  
                <div>
                    ...
                    <div class="border-b-2" v-for="(news, key) in this.newsArticle.slice(0,5)">
                        <p @click="gotoArticle(news)" class="text-sm font-thin tracking-wide text-sky-700 cursor-pointer">{{ news.title }}</p>
                ...
                <div>
                    ...
                    <div class="border-b-2" v-for="(annoucements, key) in this.announcementsArticle.slice(0,5)">
                        <p @click="gotoArticle(annoucements)" class="text-sm font-thin tracking-wide text-sky-700 cursor-pointer">{{ annoucements.title }}</p>
                ...
                <div>
                    ...
                    <div class="border-b-2" v-for="(events, key) in this.eventsArticle.slice(0,5)">
                        <p @click="gotoArticle(events)" class="text-sm font-thin tracking-wide text-sky-700 cursor-pointer">{{ events.title }}</p>
