    ...
    <div class="p-2">
      <div class="m-5 bg-white shadow-inner shadow-lg rounded px-10 py-5">
        ...
        <div class="mb-2 flex justify-start gap-1">
          ...
          <Upload :headers="{ 'x-csrf-token': token, 'X-Requested-With': 'XMLHttpRequest' }" :on-success="handleSuccess"
            :on-error="handleError" :format="['csv', 'xlsx', 'xls', 'jpg']" :multiple="false"
            :on-format-error="handleFormatError" action="/api/admin/importInventory">
          ...
          <template #bodyCell="{ text, column, record, index }">
            ...
            <template v-else-if="column.key === 'action'">
              <span>
                ...
                <Modal v-model="modal" :closable="false" :mask-closable="false" :footer-hide="true" title="Article"
                  width="1000">
                  <template #header>
            ...
            <template v-else-if="column.key === 'image'">
              <a-image style="height: 35px" :src="`/inventory/low/${record.image}`" :preview="{
