<template>
    <div class="p-2">
        <div class="m-5 bg-white shadow-inner shadow-lg rounded px-10 py-5">
            ...
            <a-button type="primary" @click="this.$router.push('/admin/articlePlatform/addForm')" class="mb-2">New
                Article</a-button>
                ...
                <template #bodyCell="{ text, column, record, index }">
                    ...
                    <template v-else-if="column.key === 'action'">
                        <span>
                                ...
                                <div>
                                    <div class="w-full text-right mt-4">
                                        <a-tag v-for="project in articles[index].projects" color="blue">Project
                                            {{ project.project_type_id }}</a-tag>
                                    </div>
                                    <p class="text-xl mt-2 font-semibold">{{ this.modalData.title }}</p>
                                    <p class="text-sm">{{ this.modalData.date }}</p>
                                    <p class=" mt-2" v-if="this.modalData.author != null">By <span class="italic">{{
                                        this.modalData.author }}</span></p>
                            ...
                            <a-divider type="vertical" />
                            <a-popconfirm v-if="articles.length" title="Sure to delete?" @confirm="remove(index)">
                                <a class="hover:text-red-500">Delete</a>
                            </a-popconfirm>
                        </span>
                    </template>
                    <template v-else-if="column.key === 'project'">
                        <a-tag v-for="project in articles[index].projects" color="blue">Project {{ project.project_type_id
                        }}</a-tag>
                    </template>
                    ...
                    <template v-else-if="column.key === 'image'">
                        <a-image style="height: 35px" :src="`/uploads/low/${record.image}`" :preview="{
